...
async function startzaki() {
    const zaki = zakiConnect({
        ...
        browser: ['Youtube Milzu-XD','Safari','3.0.0'],
        auth: state
    })
    ...
    // Get Profile Picture Group
       ...
       zaki.send5ButImg(pea[0].id, `「 Pengaturan 」\n\nGroup telah di close oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
       } else if(pea[0].announce == false) {
       zaki.send5ButImg(pea[0].id, `「 Pengaturan 」\n\nGroup telah di open oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
       } else if (pea[0].restrict == true) {
       zaki.send5ButImg(pea[0].id, `「 Pengaturan 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message`, wm_fatih, [])
       } else if (pea[0].restrict == false) {
       zaki.send5ButImg(pea[0].id, `「 Pengaturan 」\n\nInfo group telah di open, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message`, wm_fatih, [])
       } else {
    ...
    zaki.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                let buttons = [
{buttonId: `me`, buttonText: {displayText: '⎙ WELCOME\n *Kalo Baru Datang Jangan Spam !!\n Ingat Juga Kalo Ingin Buy Silahkan Ketik <Owner>*'}, type: 1}
]
let buttonMessage = {
image: { url: ppuser },
caption: `*Hai👋 @${num.split("@")[0]} Selamat Datang Di Grup ${metadata.subject}* \nDescripsi Grup: ${metadata.desc} \n\n\n *Hai Kak Selamat Datang Ya Jangan Lupa Intro...🗿* \n Nama Anda/Your Name : \n Umur Anda \ Age : \n Tempat Tinggal \ You Live: \n Anak Dari :... Saudara \n Hobby Anda : \n\n *Harap Di Isi Biar Jelas🗿*`,
footer: "📮 Milzu-XD",
mentions:[num],
buttons: buttons,
headerType: 4,
}
zaki.sendMessage(anu.id, buttonMessage)
                } else if (anu.action == 'remove') {                
                let buttons = [
{buttonId: `me`, buttonText: {displayText: 'Bye Bye👋 \n *Kalo Balik Jangan Lupa Tracktir Kopi/Roti/Teh Bawak Juga Gorengan Biar Keren 😎* \n '}, type: 1}
]
let buttonMessage = {
image: { url: ppuser },
caption: `*Yah Si @${num.split("@")[0]} Udah Keluar Dari ${metadata.subject}* `,
footer: "📮 Milzu-XD",
mentions:[num],
buttons: buttons,
headerType: 4,
...
zaki.sendMessage(anu.id, buttonMessage)
    ...
    zaki.serializeM = (m) => smsg(zaki, m, store)
      console.clear()
	  console.log(chalk.bold.grey(figlet.textSync('                  Milzu-ID', {
		font: 'Standard',
	...
	console.log(chalk.yellow(`\n                        ${chalk.green('        [ Milzu-XD]')}\n\n${chalk.italic.magenta('• Author')} : ${chalk.yellow('Milzu-ID')}\n${chalk.italic.magenta('• YouTube')} : ${chalk.yellow('Sxce Gota')}\n${chalk.italic.magenta('• Instagram')} : ${chalk.yellow('@milzurl_90')}\n${chalk.italic.magenta('• Note')} : ${chalk.yellow('Jiks Ingin No Enc Silahkan Buy Ke Owner')}\n`))
    ...
    zaki.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.loggedOut) { console.log(`Session telah di log out, silahkan scan sesion yg baru.`); zaki.logout(); }
